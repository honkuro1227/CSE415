...
def introduce():
    ...
    #introduction="\nI am M-Mobile helper. \nYou can check data, \nmake payments \nand set up AutoPay here.\nPlease tell me what you want to do today"
    return introduction
def respond(theInput):
    ...
    global count
    global countpayment
    #1todo memory pattern-> memory recent five speak
    if(theInput!=''):
    ...
    #13 random pick recent conversation
    if (len(Memory)>4):
    ...
    if match('bye',theInput.lower()):
        return "see you"
     #15 out of rule: talk about food
    else :

      ...
      if op.precond(S):
        ...
        if not (new_state in CLOSED):
            if not (new_state in OPEN):
                Priority=S.edge_distance(new_state)
                OPEN.insert(new_state,tempCost+Priority)
...
"""
    ...
    frontier.push( (node, actionList, 0), 0 )
    explored=[]

    while frontier!=None:
        node, actions, tempcost = frontier.pop()
        ...
        if(not node in explored):
            ...
            for child, direction, cost in problem.getSuccessors(node):
                frontier.push((child, actions+[direction], tempcost + cost), tempcost + cost)
